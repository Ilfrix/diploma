...
class MinIOClient:
    ...
    def upload_uploadfile(self, upload_file: UploadFile, object_path: str) -> str:
        """
    ...
    def download_file(self, object_path: str) -> bytes:
        ...
        try:
            response = self.client.get_object(config.MINIO_BUCKET_NAME, object_path)
            data = response.read()
            response.close()
    ...
    def list_files(self, prefix: str = "", recursive: bool = True) -> list:
        ...
        try:
            objects = self.client.list_objects(
                config.MINIO_BUCKET_NAME, prefix=prefix, recursive=recursive
            )
            for obj in objects:
                files.append(
                    {
    ...
    def copy_file(self, source_path: str, dest_path: str) -> str:
        ...
        try:
            self.client.copy_object(
                ...
                source={"bucket": config.MINIO_BUCKET_NAME, "object": source_path},
            )
            return dest_path
        except S3Error as e:
            raise HTTPException(
