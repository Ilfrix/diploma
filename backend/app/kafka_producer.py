...
class KafkaProducerManager:
    ...
    async def send_image_for_processing(
        self, image_id: str, image_bytes: bytes, metadata: dict[str, Any]
    ):
        ...
        await self.send_message(
            topic=config.KAFKA_IMAGE_TOPIC, key=image_id, value=message
        )
        logger.info(f"Image {image_id} sent for processing")

    async def send_search_request(
        self, request_id: str, image_bytes: bytes, metadata: dict[str, Any]
    ):
