        ...
        try:
            if utility.has_collection(self.collection_name):
                ...
                fields = [
                    ...
                    FieldSchema(name="sample_id", dtype=DataType.VARCHAR, max_length=255),
                    FieldSchema(name="crop_index", dtype=DataType.INT32),
                    
                    # Информация о пользователе
                    FieldSchema(name="user_id", dtype=DataType.VARCHAR, max_length=255),
                    FieldSchema(name="original_id", dtype=DataType.VARCHAR, max_length=255),
                    ...
                    FieldSchema(name="mime_type", dtype=DataType.VARCHAR, max_length=100),
                ...
                schema = CollectionSchema(
                    fields, 
                    description="Image crop vectors for duplicate detection"
                )
            ...
            logger.info(f"Collection {self.collection_name} loaded, contains {self.collection.num_entities} entities")
        ...
        try:
            ...
            for v_data in vectors_data:
                ...
                if len(vector_list) != self.dim:
                    logger.warning(f"Vector dimension mismatch: expected {self.dim}, got {len(vector_list)}")
                    if len(vector_list) > self.dim:
