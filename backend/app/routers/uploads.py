...
async def get_uploaded_image(
    ...
    # Попытка найти семпл по ID
    sample = (
        db.query(Sample)
        .filter(Sample.id == image_id, Sample.user_id == current_user.id)
        .first()
    )
    ...
    # Если не получилось через семпл, поиск по паттерну
    if not object_path:
...
async def get_sample_image(
    ...
    image_bytes = minio_client.download_file(image_path)
    # Опредлеение расширения для имени файла
    extension = "jpg"
...
async def get_batch_presigned_urls(
    ...
    for sample_id in ids:
        ...
        if (
            sample
            and sample.image
            and minio_client.file_exists(sample.image.image_path)
        ):
...
async def check_image_exists(
    ...
    if sample and sample.image:
        ...
        # Попытка найти напрямую в таблице Image
    ...
    return {"exists": False, "image_id": image_id}
...
async def get_image_by_id(
    ...
    # Проверка, есть ли у пользователя семпл с этим изображением
    sample = (
        db.query(Sample)
        .filter(Sample.image_id == image_id, Sample.user_id == current_user.id)
        .first()
    )

    if not sample:
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN, detail="Access denied to this image"
        )
