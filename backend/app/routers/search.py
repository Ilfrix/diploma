...
async def get_similar(
    ...
    for sample_id, data in sorted(
        ...
        if similar_sample and similar_sample.image and similar_sample.image_id:
...
async def get_sample_vectors(
    ...
    crops_with_vectors = (
        db.query(Crop, VectorModel)
        .join(VectorModel, Crop.id == VectorModel.crop_id)
        .filter(Crop.image_id == sample.image_id)
        .all()
    )
    ...
    for vec_id, score, metadata in similar_vectors:
        ...
        if similar_sample_id:
            similar_sample = (
                db.query(Sample)
                .filter(
                    Sample.id == similar_sample_id, Sample.user_id == current_user.id
                )
                .first()
            )
...
async def search_similar_by_image(
    ...
    query = (
        ...
        .filter(Sample.id.in_(found_sample_ids))
        .all()
    )
...
async def get_search_result(
    ...
    search_request = (
        db.query(SearchRequest).filter(SearchRequest.id == request_id).first()
    )
