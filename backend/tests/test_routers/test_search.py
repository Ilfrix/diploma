...
class TestSearchRouter:
    def test_search_similar_by_sample(
        ...
        image = ImageModel(
            image_path="test/path.jpg", image_hash="hash123", mime_type="image/jpeg"
        )
        ...
        vector = Vector(crop_id=crop.id, milvus_id="test_milvus_id")
    ...
    def test_get_sample_crops(
        ...
        with patch("app.routers.samples.minio_client", mock_minio):
            response = client.get(
                f"/api/samples/{sample.id}/crops", headers=auth_headers
            )
    ...
    def test_search_similar_by_image_upload(
        self, client, auth_headers, test_image_bytes
    ):
    ...
    def test_get_sample_vectors(self, client, auth_headers, db_session, test_user):
        ...
        vector = Vector(crop_id=crop.id, milvus_id="milvus_123")
        db_session.add(vector)
        db_session.commit()

        response = client.get(f"/api/samples/{sample.id}/vectors", headers=auth_headers)
