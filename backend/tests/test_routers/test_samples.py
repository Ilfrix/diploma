...
class TestSamplesRouter:
    def test_create_sample_success(
        self, client, auth_headers, test_image_bytes, mock_minio
    ):
        ...
        with patch("app.routers.samples.minio_client", mock_minio):
            ...
            data = {"name": "Test Sample", "description": "Test Description"}

            response = client.post(
                "/api/samples", headers=auth_headers, files=files, data=data
            )
    ...
    def test_create_sample_duplicate(
        self,
        client,
        auth_headers,
        test_image_bytes,
    ...
    def test_get_sample(self, client, auth_headers, db_session, test_user):
        ...
        response = client.get(f"/api/samples/{sample.id}", headers=auth_headers)
    ...
    def test_get_sample_not_found(self, client, auth_headers):
        ...
        response = client.get("/api/samples/nonexistent-id", headers=auth_headers)
    ...
    def test_list_samples(self, client, auth_headers, db_session, test_user):
        ...
        for i in range(3):
            sample = Sample(
                user_id=test_user.id, name=f"Sample {i}", status=ProcessStatus.PROCESSED
            )
            db_session.add(sample)
        db_session.commit()
    ...
    def test_list_samples_with_filter(
        self, client, auth_headers, db_session, test_user
    ):
        """Список семплов, фильтр по статусу."""
        sample1 = Sample(
            user_id=test_user.id, name="Pending Sample", status=ProcessStatus.PENDING
        )
        sample2 = Sample(
            user_id=test_user.id,
        ...
        response = client.get(
            "/api/samples?status_filter=pending", headers=auth_headers
        )
    ...
    def test_update_sample(self, client, auth_headers, db_session, test_user):
        """Обновление семпла"""
        sample = Sample(
            user_id=test_user.id, name="Original Name", status=ProcessStatus.PROCESSED
        )
        db_session.add(sample)
        db_session.commit()
    ...
    def test_delete_sample(
        ...
        """Удаление семпла"""
        sample = Sample(
            user_id=test_user.id, name="To Delete", status=ProcessStatus.PROCESSED
        )
        db_session.add(sample)
        db_session.commit()

        response = client.delete(f"/api/samples/{sample.id}", headers=auth_headers)
    ...
    def test_get_sample_stats(self, client, auth_headers, db_session, test_user):
        ...
        samples = [
            Sample(
                user_id=test_user.id, name="Sample 1", status=ProcessStatus.PROCESSED
            ),
            Sample(user_id=test_user.id, name="Sample 2", status=ProcessStatus.PENDING),
            Sample(
                user_id=test_user.id, name="Sample 3", status=ProcessStatus.PROCESSED
            ),
        ...
        response = client.get("/api/samples/stats/summary", headers=auth_headers)
