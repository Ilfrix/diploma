...
class TestUploadsRouter:
    def test_get_sample_image_redirect(
    ...
    def test_get_presigned_url(
        ...
        with patch("app.routers.uploads.minio_client", mock_minio):
            mock_minio[
                "url"
            ].return_value = "https://minio.example.com/presigned/test.jpg"
    ...
    def test_download_sample_image(
        ...
        with patch("app.routers.uploads.minio_client", mock_minio):
            ...
            response = client.get(
                f"/api/uploads/sample/{sample.id}/download", headers=auth_headers
            )
    ...
    def test_get_image_info(
        ...
        with patch("app.routers.uploads.minio_client", mock_minio):
            ...
            response = client.get(
                f"/api/uploads/sample/{sample.id}/info", headers=auth_headers
            )
    ...
    def test_check_image_exists(
        ...
        response = client.get(f"/api/uploads/check/{sample.id}", headers=auth_headers)
    ...
    def test_check_image_not_exists(self, client, auth_headers):
        ...
        response = client.get("/api/uploads/check/nonexistent-id", headers=auth_headers)
