...
class LoadTester:
    ...
    def register_if_needed(self) -> bool:
        ...
        response = requests.post(f"{API_BASE_URL}/api/register", json=TEST_USER)
    ...
    def create_unique_image(self, index: int) -> bytes:
        ...
        if index % 2 == 0:
            ...
            draw.ellipse(
                [x - r_radius, y - r_radius, x + r_radius, y + r_radius],
                outline=(255, 255, 255),
                width=3,
            )
    ...
    def create_sample(
        self, name: str, image_bytes: bytes, retry_count: int = 2
    ) -> dict:
        """Создание одного сэмпла с повторами при ошибках"""
        headers = {"Authorization": f"Bearer {self.access_token}"}
        files = {"image": ("test.jpg", image_bytes, "image/jpeg")}
        data = {"name": name, "description": f"Test sample {name}"}
        ...
        for attempt in range(retry_count):
            try:
                ...
                if attempt < retry_count - 1:
                    time.sleep(0.5)
                    continue
                return {"success": False, "error": str(e)[:100], "time": 0}
    ...
    def create_samples_batch(self, count: int, delay_between: float = 0.05) -> dict:
        ...
        for i in range(count):
            ...
            result = self.create_sample(
                f"Test_Sample_{i + 1}_{uuid.uuid4().hex[:4]}", image_bytes
            )
    ...
    def run_single_test(self, count: int, iteration: int) -> dict:
        ...
        print(
            f"✓ Успешно: {create_result['successful']}/{count}, "
    ...
    def run_load_test(self, sample_counts: list[int], iterations: int = 10):
        ...
        for count in sample_counts:
            ...
            avg_result = {
                ...
                "std_successful": statistics.stdev(successful_counts)
                if len(successful_counts) > 1
                else 0,
                ...
                "std_time_per_sample": statistics.stdev(avg_times)
                if len(avg_times) > 1
                else 0,
                "average_total_time": statistics.mean(total_times),
                "std_total_time": statistics.stdev(total_times)
            ...
            print(
                f"    Успешно создано (ср): {avg_result['average_successful']:.1f} ± {avg_result['std_successful']:.1f}"
            )
            print(
                f"    Успешность: {avg_result['average_success_rate']:.1f}% ± {avg_result['std_success_rate']:.1f}%"
            )
            print(
                f"    Время на сэмпл: {avg_result['average_time_per_sample']:.3f} ± {avg_result['std_time_per_sample']:.3f} сек"
            )
            print(
                f"    Общее время: {avg_result['average_total_time']:.2f} ± {avg_result['std_total_time']:.2f} сек"
            )
    ...
    def analyze_complexity(self, results: dict):
        ...
        print(
            f"{'Кол-во':<8} {'Время/сэмпл':<18} {'Рост времени':<18} {'Общее время':<18} {'Рост общего':<18}"
        )
        print("-" * 80)
        ...
        for i, count in enumerate(counts):
            ...
            if i > 0:
                ...
                total_time_growth = (
                    total_times[i] / total_times[i - 1] if total_times[i - 1] > 0 else 0
                )
            ...
            print(
                f"{count:<8} {time_str:<18} {time_growth_str:<18} {total_time_str:<18} {total_growth_str:<18}"
            )

        # Определение сложности
        if len(counts) >= 2:
            time_growths = [
                times[i + 1] / times[i] for i in range(len(times) - 1) if times[i] > 0
            ]
            count_growths = [counts[i + 1] / counts[i] for i in range(len(counts) - 1)]

            if time_growths:
                avg_growth_ratio = statistics.mean(
                    [t / c for t, c in zip(time_growths, count_growths, strict=True)]
                )

                print("\n📐 Теоретическая оценка временной сложности:")
                print(
                    f"  Среднее отношение роста времени к росту данных: {avg_growth_ratio:.3f}"
                )

                if avg_growth_ratio > 1.2:
                    complexity = "O(n²) или выше (суперлинейный рост)"
                    print(
                        "  ⚠ Время растет быстрее количества данных - возможны проблемы с производительностью"
                    )
                    ...
                    print(
                        "  ✓ Хорошая производительность, время растет медленнее данных"
                    )
                else:
                    complexity = "O(1) - константная сложность"
                    print(
                        "  ✓ Отличная производительность, время не зависит от объема данных"
                    )
                ...
                if len(counts) >= 2:
                    ...
                    last_count = counts[-1]
                    slope = (
                        (times[-1] - times[0]) / (counts[-1] - counts[0])
                    ...
                    print(
                        f"  При {last_count * 2} сэмплах: ~{last_time + slope * last_count:.4f} сек/сэмпл (при линейном росте)"
                    )
...
def main():
    ...
    try:
        ...
        results = tester.run_load_test(
            sample_counts=sample_counts, iterations=iterations
        )
